...
struct Dhu_airline
{
	...
	int seat_num;
	char email[15];
	struct Dhu_airline *following;
}
*begin, *stream;
struct Dhu_airline *dummy;
...
void reserve(int x)
{
	...
	{
		// first user
		begin = stream = (struct Dhu_airline*)malloc(sizeof(struct Dhu_airline));
		details();
	...
	else
	{
		...
		stream->following = (struct Dhu_airline *)malloc(sizeof(struct Dhu_airline));
		stream = stream->following;
		details();
		stream->following = NULL;
...
void savefile()
{
	FILE *fpointer = fopen("Dhu_airline records", "w");
	if (!fpointer)
	{
	...
	printf("\n\n\t Details have been saved to a file (Dhu_airline record)");
	fclose(fpointer);
}
//********************************GOOD LUCK Dhu_airline ***************************************
